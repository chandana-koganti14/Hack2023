...
$status=2;
$cancelby='a';
$sql = "UPDATE tblbook SET status=:status,CancelledBy=:cancelby WHERE  BookingId=:bid";
...
$status=1;
$cancelby='a';
$sql = "UPDATE tblbook SET status=:status WHERE BookingId=:bcid";
$query = $dbh->prepare($sql);
$query -> bindParam(':status',$status, PDO::PARAM_STR);
...
<html>
<head>
<title>VNITSW | Admin manage Bookings</title>
				...
				<div class="agile-tables">
					<div class="w3l-table-info">
					  <h2>Manage Bookings</h2>
						...
						<tbody>
<?php $sql = "SELECT tblbook.BookingId as bookid,info.name as fname,info.email as email,tbl.PackageName as pckname,tblbook.PackageId as pid,tblbook.Comment as comment,tblbook.status as status,tblbook.CancelledBy as cancelby,tblbook.UpdationDate as upddate from info join  tblbook on  tblbook.email=info.email join tbl on tbl.PackageId=tblbook.PackageId";
$query = $dbh -> prepare($sql);
$query->execute();
